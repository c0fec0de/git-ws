...
BoolOptions: TypeAlias = tuple[tuple[str, bool], ...]
...
class Git:
    ...
    def clone(self, url, revision: Optional[str] = None, depth: Optional[int] = None):
        ...
        # * we reuse a filesystem cache for clones.
        if self.clone_cache:
    ...
    def fetch(self, shallow: Optional[str] = None, unshallow: bool = False):
        ...
        assert not shallow or not unshallow, "shallow and unshallow are mutually exclusive"
        if shallow:
